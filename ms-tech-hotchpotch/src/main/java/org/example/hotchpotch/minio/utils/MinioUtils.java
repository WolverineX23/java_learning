package org.example.hotchpotch.minio.utils;
...
import org.example.hotchpotch.minio.enums.ExceptionEnums;
import org.example.hotchpotch.minio.handler.GeneralException;
...
//                items.add(JSON.parse(String.format(format, "/".concat("test").concat("/").concat(item.objectName()), formatFileSize(item.size()))));
    ...
    public String getPresignedObjectUrl(String bucketName, String fileName, Integer expire) {
        ...
        expire = Objects.isNull(expire) ? minioConfig.getExpire() : expire;
        // 验证桶是否存在在
